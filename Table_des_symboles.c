...
void remove_block_symboles() {
	elem *tracker = storage;
	elem *deleted; //disallocate the elem removed from storage 
	while(tracker->symbol_value->int_val < tracker->next->symbol_value->int_val) {
		deleted = tracker;
		storage = tracker->next;
		free(deleted);
	}
	if(!tracker->symbol_value->int_val){
		deleted = tracker;
		storage = tracker->next;
		free(deleted);
	}
